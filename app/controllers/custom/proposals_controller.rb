class ProposalsController < ApplicationController
  ...
  before_action :authenticate_user!, except: [:index, :show, :map, :summary, :create, :created, :new, :publish, :share]
  ...
  def create
    if current_user
      ...
      if user
        ...
        flash[:error] = "Ooops! User not found"
  ...
  def publish
    Rails.logger.info "About to try to publish proposal"
    @proposal.publish
  ...
  private
    ...
    user = User.find_by(email: Rails.application.secrets[:anonymous_user_settings][:email])
    user ||= create_anonymous_user if user.nil?
    user
  end

  def create_anonymous_user
    anon_credentials = Rails.application.secrets[:anonymous_user_settings]

    User.create(
      username: anon_credentials[:user],
      email: anon_credentials[:email],
      password: anon_credentials[:password],
      password_confirmation: anon_credentials[:confirmed_password],
      document_number: anon_credentials[:document_id],
