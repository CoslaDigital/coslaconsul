...
#      @user = current_user || identity.user || User.first_or_initialize_for_oauth(auth)
      ...
      if save_user
        ...
        method = (provider == :saml) ? "MyAccount" : provider.to_s.capitalize
        Rails.logger.error("message is  (#{:success}) (#{provider.to_s.capitalize} #{method})")
        set_flash_message(:notice, :success, kind: method) if is_navigational_format?
      else
